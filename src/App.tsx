...
export default function App() {
    ...
    useEffect(() => {
        ensureServiceWorkerReady().catch((serviceWorkerError) => {
            setStatusMessage(
                buildServiceWorkerErrorMessage(serviceWorkerError)
            );
        });
    }, []);

    const handleDirectorySelection: ChangeEventHandler<
        HTMLInputElement
    > = async (directoryInputChangeEvent) => {
        ...
        const virtualFileDescriptors =
            await createVirtualFileDescriptors(selectedFileList);
    ...
    const executeRunner = async () => {
        ...
        if (
            ...
            setStatusMessage(
                APPLICATION_ERROR_MESSAGES.serviceWorkerPingFailure
            );
        ...
        if (!entryModuleResponse || !entryModuleResponse.ok) {
            setStatusMessage(
                buildEntryNotReachableMessage(absoluteEntryModuleUrl)
            );
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                >
                    {javaScriptFilePaths.map((javaScriptFilePath) => (
                        <option
                            key={javaScriptFilePath}
                            value={javaScriptFilePath}
                        >
                            {javaScriptFilePath}
                        </option>
                    ))}
